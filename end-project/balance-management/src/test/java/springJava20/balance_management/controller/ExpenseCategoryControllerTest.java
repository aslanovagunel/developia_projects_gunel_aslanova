...
public class ExpenseCategoryControllerTest {
	...
	private static final String END_POINT_PATH = "/expense-category";
	...
	public void testAddShouldReturn201Created() throws Exception {
		ExpenseCategoryAddRequest r = new ExpenseCategoryAddRequest(LocalDate.of(2025, 06, 03), "name");

		String requestBody = objectMapper.writeValueAsString(r);
		Mockito.when(expenseCategoryService.add(r)).thenReturn(1);

		mockMvc.perform(post(END_POINT_PATH).with(csrf()).contentType("application/json").content(requestBody))
				.andExpect(status().isCreated()).andDo(print());

		Mockito.verify(expenseCategoryService, times(1)).add(r);
	}
	...
	void testGetAllExpenseCategories() throws Exception {
	    ...
	    ExpenseCategoryListResponse mockResponse = new ExpenseCategoryListResponse();
		List<ExpenseCategorySingleResponse> responses = new ArrayList<>();
		ExpenseCategorySingleResponse singleResponse = new ExpenseCategorySingleResponse();
		...
		when(expenseCategoryService.getAll(begin, length)).thenReturn(mockResponse);

		mockMvc.perform(get("/expense-category/begin/{begin}/length/{length}", begin, length))
				.andExpect(status().isOk()).andExpect(jsonPath("$.responses").isArray())
		...
		verify(expenseCategoryService, times(1)).getAll(begin, length);
	}

	@WithMockUser
	@Test
	public void testUpdateShouldReturn200Ok() throws Exception {
		ExpenseCategoryUpdateRequest request = new ExpenseCategoryUpdateRequest(1, "Updated Name");

		String requestBody = objectMapper.writeValueAsString(request);
		Mockito.doNothing().when(expenseCategoryService).update(request);

		mockMvc.perform(put(END_POINT_PATH).with(csrf()).contentType("application/json").content(requestBody))
				.andExpect(status().isOk()).andDo(print());

		Mockito.verify(expenseCategoryService, times(1)).update(request);
	}
	...
	public void testDeleteExpenseCategory() throws Exception {
		...
		Mockito.doNothing().when(expenseCategoryService).deleteById(id);

		mockMvc.perform(delete(END_POINT_PATH + "/" + id).with(csrf())).andExpect(status().isOk()).andDo(print());

		Mockito.verify(expenseCategoryService, times(1)).deleteById(id);
	}
}
